...
class OrderController extends Controller
{
	...
	public function update_order_qty(Request $request){
        ...
        foreach($data['order_product_id'] as $key => $pro){
        ...
        Mail::send('admin.mail_order_confirm.mail_order_confirm',  ['cart_array'=>$cart_array, 'shipping_array'=>$shipping_array ,'code'=>$code_mail ] , function($message) use ($title_mail,$data){
        ...
        $statistical = Statistical::where('order_date',$order_date)->get();/*so sánh thời gian giữa 2 bang order_date và access*/
        if($statistical){
    ...
    public function view_order_history(Request $request,$order_code){
        if(!Session::get('customer_id')){
            ...
            //lich sử đơn hàng
            $order_details = OrderDetails::with('product')->where('order_code',$order_code)->get();/*lấy ra sản phẩm theo order code*/
            $order = Order::where('order_code',$order_code)->first();
