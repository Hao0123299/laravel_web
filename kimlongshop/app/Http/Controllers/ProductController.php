...
class ProductController extends Controller
{
    ...
    public function save_product(Request $request){
        ...
        $numberPriceCost = filter_var($request->product_price, FILTER_SANITIZE_NUMBER_INT);/*lấy ký tự số*/
    ...
    public function update_product(Request $request,$product_id){
        ...
        if($get_image){
                    ...
                    $new_image =  $name_image.rand(0,99).'.'.$get_image->getClientOriginalExtension(); /*random tên hình ảnh để thêm hình ảnh cũ vào không bị trùng tên*/
    ...
    public function details_product($product_slug , Request $request){
        ...
        //thêm hình ảnh
        $picture = Picture::where('product_id', $product_id)->get(); /*lấy hết hình ảnh*/
        /*$picture = Picture::where('product_id', $product_id)->take(3)*/; /*lấy số lượng hình ảnh nhất định*/
        ...
        $rate = Rate::where('product_id', $product_id)->avg('rate_rate'); /*lấy ra số sao được vote dựa trên product */
                                        ...
                                        foreach($comment as $key => $c_rep)
                                        {
                                            if($c_rep->comment_comment_prarent == $c->comment_id)
                                            {
                                                ...
                                                <div class="row binh_luan " style="margin-left: 10px; background: white ">
                                                    <div class="col-sm-2" >
                                                        <img width="50%" src="'.url('/public/frontend/images/admin.jpg').'" class="img img-responsive">
                                                    </div>
                                                    <div class="col-sm-10">
