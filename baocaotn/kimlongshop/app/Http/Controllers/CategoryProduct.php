...
class CategoryProduct extends Controller
{
    ...
    public function edit_category_product($category_product_id){
        ...
        $edit_category_product = DB::table('tbl_category_product')
            ->where('category_id',$category_product_id)
            ->get();
        $category = CategoryProductModel::orderBy('category_id','DESC')
            ->get();
        $cate_product = DB::table('tbl_category_product')
            ->orderby('category_id','desc')
            ->get();
    ...
    public function add_category_product_parent(){
        $this->AuthLogin();
        $category = CategoryProductModel::where('category_parent', 0)
            ->orderBy('category_id','DESC')
            ->get();
        ...
        $brand_product = DB::table('tbl_brand')
            ->orderby('brand_id','desc')
            ->get();
        return view('admin.add_category_product_parent')
            ->with('cate_product', $cate_product)
            ->with('brand_product',$brand_product)
            ->with('category',$category);
    }
    public function save_category_product_parent(Request $request){
        $this->AuthLogin();
        $data = array();
